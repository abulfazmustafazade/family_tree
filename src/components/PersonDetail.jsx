    ...
    <div className="space-y-4">
        ...
        <div className="mt-2 space-y-1 text-sm text-stone-700">
          {person.birthDate && (
            <div className="flex items-center gap-2">
              ...
              Doğum: {fmtDate(person.birthDate)}
          ...
          {!person.isAlive && person.deathDate && (
            <div className="flex items-center gap-2">
              <Calendar className="w-3.5 h-3.5 shrink-0" />
              Vəfat: {fmtDate(person.deathDate)}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-1.5 text-sm">
        {father && (
          <div>
            ...
            <span className="text-stone-800">{father.firstName} {father.lastName}</span>
          </div>
        )}
        {mother && (
          <div>
            <span className="text-stone-500">Anası: </span>
            <span className="text-stone-800">{mother.firstName} {mother.lastName}</span>
          </div>
        )}
        {spouse && (
          <div>
            <span className="text-stone-500">Həyat yoldaşı: </span>
            <span className="text-stone-800">{spouse.firstName} {spouse.lastName}</span>
          </div>
        )}
        {children.length > 0 && (
          <div>
      ...
      <div className="flex flex-col gap-2 pt-1">
        ...
        <button
          ...
          className="w-full px-4 py-2.5 bg-stone-800 hover:bg-stone-900 text-white
            text-sm font-medium rounded-md flex items-center justify-center gap-2 transition-colors"
        >
          🌿 Bu şəxsdən şəcərəyə bax
        </button>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 px-4 py-2.5 bg-amber-700 hover:bg-amber-800 text-white
              text-sm font-medium rounded-md flex items-center justify-center gap-2 transition-colors"
          >
          ...
          <button
            onClick={onClose}
            className="px-4 py-2.5 bg-stone-200 hover:bg-stone-300 text-stone-800
